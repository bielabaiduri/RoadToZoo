...
<?php
  ...
  $id = $_SESSION['user_id'];
  $sql="SELECT * FROM staff WHERE Staffic='$id'"; 
  $result = mysqli_query($dbc,$sql) or die('Query failed. ' . mysqli_error());
  $row = mysqli_fetch_assoc($result);

?>

<form name="form1" id="form1" method="post" action="profilestaff.php">
    ...
    <table class="table table-condensed" border="0" style="width:600px">
        <tr>
          ...
          <td>
            <div class='col-lg-12'>
                 <input class="form-control" type="text" name="ic" id="ic" value="<?php echo $row['Staffic']?>" readonly/> 
            </div>
          </td>
        </tr>
        <tr>
          ...
          <td>
            <div class='col-lg-12'>
                <input type='text' name="name" id="name" class="form-control" value="<?php echo $row['Staffname']?>" required/>
            </div>
          </td>
        </tr>
        <tr>
  ...
  <input name="staffid" value="<?php echo $row['Staffid']?>" type="hidden" >
